...
class Archiver extends \Piwik\Plugin\Archiver
{
    ...
    public static $ISROOTARCHIVE = true;
    ...
    public function __construct(ArchiveProcessor $processor)
    {
        ...
        $this->maximumRowsInDataTable = $configuration->getMaximumRowsInDataTable();
        $this->maximumRowsInSubTable = $configuration->getMaximumRowsInSubTable();
    ...
    public function aggregateDayReport()
    {
        ...
        $this->archiveUserCountry();
    }

    private function archiveUserCountry()
    {
        if (!\Piwik\Plugins\MediaAnalytics\Segment::archiveSegmentOnDemandSupported()) {
            return;
        }

        if (!self::$ISROOTARCHIVE) {
            // we stop any possible recursion.
            return;
        }

        self::$ISROOTARCHIVE = false;

        try {
            $currentSegment = $this->getParams()->getSegment()->getString();
            $mediaSegment = BaseWidget::addMediaSegment($currentSegment);

            $site = $this->getParams()->getSite();
            $idSites = array($site->getId());

            $segment = new \Piwik\Segment($mediaSegment, $idSites);

            if (ArchiveProcessor\Rules::isSegmentPreProcessed($idSites, $segment)) {
            ...
            $archiveLoader->prepareArchive('UserCountry');
        } catch (\Exception $e) {
            self::$ISROOTARCHIVE = true;
            throw $e;
        }
